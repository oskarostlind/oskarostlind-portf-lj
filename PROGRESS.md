...
**Sajten är live.** https://oskarostlind.vercel.app
...
- **HTTPS.** Vercel utfärdar certifikatet automatiskt när domänen verifierats; vid kontrolltillfället svarade http men inte https ännu. Det brukar lösa sig inom minuter till någon timme. Aktivera **inte** Stratos "Kryptera" parallellt — två certifikatutfärdare på samma domän ställer bara till det.
- **DNS-propagering** kan enligt Strato ta upp till 24 timmar. Vår upplösning gick igenom direkt.
- **`www.oskarostlind.se`** är inte tillagd. Vill du fånga den: lägg till den i Vercel som redirect till apex, och sätt `CNAME www → cname.vercel-dns.com` hos Strato.
