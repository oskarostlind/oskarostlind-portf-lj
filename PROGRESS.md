...
- [x] **Casebilder** — sju automatgenererade skärmbilder ligger i `public/case/`.
- [x] **Cal.com och Cloudflare borttagna** — struket på Oskars begäran.

## Kvar att göra

- [ ] **`git rm components/ui/Booking.tsx components/ui/Analytics.tsx`** — filerna är tömda men ligger kvar, eftersom byggsandlådan inte får radera filer i den molnmonterade mappen. Inget importerar dem, så de påverkar varken bundle eller bygge. Rensa vid nästa commit.
- [ ] **Resultatsiffror och kundcitat** — se "Luckor". Det enda som återstår av substans, och det som skulle lyfta portföljen mest. Kräver uppgifter från Oskar.
...
- **Performance stannar på ~88, och det är ett beslut — inte en obetald skuld.** Oskars val 2026-08-08 efter att orsaken lagts fram. Kravet i specen är ≥ 90 och sajten mäter 87–89 i produktion. Hela gapet ligger i hero-rubrikens avslöjande: LCP-elementet är rubrikens andra rad, som har `transition-delay` 370 ms plus 1 s övergång. Med FCP på ~1,1 s landar LCP på ~2,5 s, vilket stämmer med de tre produktionsmätningarna (2,46 / 2,71 / 2,74 s). Rubriken är alltså inte långsam att hämta, bara långsam att visa sig — sidan är färdig långt innan. Att korta fördröjningarna till 80–200 ms och övergången till 600 ms hade gett LCP ~1,9 s och sannolikt ≥ 90. Det valdes bort: anslaget i hero är sajtens signatur och det enda en besökare faktiskt upplever, medan 88 mot 90 är en tröskel i ett mätverktyg som ingen människa märker. **Ändra alltså inte hero-timingen i jakt på siffran.** Vill man ändå ha den är det `transition-delay` på raderna i `components/sections/Hero.tsx` och `transition: transform 1s` i `.reveal-line > span` i `app/globals.css` som gäller.
...
- **~~Bilder~~** — löst 2026-08-08. Sju automatgenererade skärmbilder ligger i `public/case/`. Kör om genereringen när ett projekt ändrar utseende.
- **~~Cal.com~~ och ~~Cloudflare~~** — borttagna 2026-08-08, inte längre luckor.
- **`iOS Debug`-widget i produktion på AvyraCards** — syns nere till höger på `avyracards.se`. Utanför det här repot, men värd att åtgärda.
- **Trasig logotypbild i AvyraCards header** — alt-texten "Avy" renderas i stället för bilden. Även den utanför repot.
...
- **2026-08-08 (natt, andra passet)** — **Casebilderna på plats, Cal.com och Cloudflare borta.**
  **Bilderna genereras, de ritas inte för hand.** Oskars förslag var att bädda in varje projekt som en levande iframe. Tekniskt möjligt — ingen av de sju sätter `X-Frame-Options` eller `frame-ancestors` — men det hade betytt sju kompletta webbplatser som laddas och hydrerar hos varje besökare, alltså en prestandakostnad tiotals gånger större än de två poäng han just valt att offra för hero-animationen. Dessutom renderar en iframe mot sin egen bredd, så korten hade visat projektens *mobilvyer*, och portföljen hade blivit lika trasig som svagaste projektet. Idén är i stället flyttad till byggtid: `shoot.mjs` kör headless Chrome mot varje live-URL och sparar 1600×1000 WebP i `public/case/`. Samma verkliga sida, renderad en gång i stället för en gång per besökare. Skriptet ligger inte i repot — det är ett engångsverktyg, kör om det när ett projekt ändrats.
  Två av sju behövde ett andra pass. **AvyraCards** fick med en kakruta och en **`iOS Debug`-widget** — den senare är ett riktigt fynd: en utvecklarwidget som ligger i produktion på flaggskeppsprodukten. Löst i bilden genom att klicka "Neka" (integritetsbevarande valet) och ta bort kvarvarande fasta overlays. **Escape room** hade 45 % tom yta eftersom sidan är kort, och en trasig emoji-glyf — den senare var sandlådans Chrome som saknade emoji-typsnitt, inte ett fel på sajten. Löst med `fonts-noto-color-emoji` och en zoomad 1120×700-vy uppskalad till 1600×1000. Alla sju har nu identiska mått, vilket krävs eftersom korten beskär med `object-cover` mot fast bildförhållande — olika mått hade gett olika mycket bortklippt.
  Kvar att notera: **AvyraCards logotyp i headern är en trasig bild** i produktion (alt-texten "Avy" syns i stället för bilden). Syns i båda passen, alltså inte ett artefakt. Ligger utanför det här repot men bör lagas.
  **Cal.com** struket: `Booking.tsx`, anropet i `/kontakt`, `booking`-blocken i båda språkfilerna, `calLink` i `lib/site.ts` och variabeln i `.env.example`, `.env.local` och README. **Cloudflare Web Analytics** likaså — Oskar har inget behov av besöksstatistik nu. Vercels egen är den naturliga vägen senare; den kräver `@vercel/analytics` och en komponent, alltså lika mycket jobb, och Vercels siffror utan paketet är infrastrukturmätning, inte besökare. Motiveringen ligger som kommentar i `lib/site.ts`.
  Passade på att rätta två stale-fel i README: en mening som pekade på fel miljövariabler, och en deploy-sektion som fortfarande rekommenderade Cloudflare Pages och angav apex-IP `76.76.21.21` — fel host och fel IP mot vad som faktiskt är satt.
  Verifierat i lokal produktionsserver: 38 `_next/image`-taggar och fyra blur-platshållare på startsidan, alla sju casesidor visar sin skärmbild med lokaliserad alt-text, `/kontakt` innehåller noll träffar på cal.com men fortfarande sitt formulär, och inget beacon-skript laddas. `bash scripts/build.sh` går igenom utan fel; `case-images: 7 bild(er) via sharp`.
- **2026-08-08 (natt)** — **Allt pushat och deployat; mätt mot skarpa sajten.** Verifierat i produktion: `/arbeten/socialcard` och `/en/work/socialcard` ger 308 till `avyracards`, casesidan svarar 200 med `<title>AvyraCards — Oskar Östlind</title>` och live-länken pekar på `avyracards.se`. Inga `rel="stylesheet"` kvar i dokumentet, logotypens `aria-label` och manifestets `opacity-[0.42]` är på plats.
  **Lighthouse mobil mot `https://oskarostlind.se`, tre körningar: Accessibility 100, Best Practices 100, SEO 100 i samtliga.** Tre av fyra krav i specen är därmed uppfyllda och verifierade — och A11y gick från 96 till 100. Performance mätte 70 / 88 / 89. Sjuan är sandlådans delade CPU som slår igenom i TBT (1331 ms i den körningen mot 232 och 348 i de andra), inte sajten; den siffran ska ignoreras. **LCP däremot är stabilt: 2,46 / 2,71 / 2,74 s** — och det är den som avgör. Orsaken är utredd och ligger i hero-rubrikens timing, inte i hämtning eller bundlar. Oskar valde att behålla animationen och acceptera ~88; se "Beslut". Prestandapunkten är därmed stängd, inte överlämnad.
  PageSpeed Insights som andrahandskälla gick inte att få ut: API:t utan nyckel svarar 429 (dagskvot 0) och webb-UI:t fastnade på "Analys pågår" i över tre minuter utan att leverera. Vill man ha Googles egen siffra får den hämtas manuellt från en webbläsare.
- **2026-08-08 (sent)** — **Flaggskeppscaset heter AvyraCards, inte SocialCard — och länkade till en död domän.** Oskar bekräftade att SocialCard är produktens gamla namn. Kontrollerat mot verkligheten: `avyracards.se` och `avyracards.com` svarar 200 och sajten kallar sig `AvyraCards` i sin egen `<title>`, medan `socialcard.se` slår upp till en Strato-platshållare (217.160.0.138) och inte svarar över https alls. Den låg som `live` på caset, alltså en trasig "besök sajten"-knapp på portföljens starkaste referens. Titeln är nu `AvyraCards` (Oskars val — historiken om Flask-versionen och omskrivningen står kvar i brödtexten, där den säljer bättre), slugen `socialcard` → `avyracards` och live-länken pekar på `avyracards.se`. Gamla adressen 308:as i `next.config.ts` för båda språken. Verifierat i lokal produktionsserver: `/arbeten/socialcard` och `/en/work/socialcard` ger 308 till rätt mål, nya adresserna 200, sitemapen listar `avyracards` och casesidans `<title>` säger "AvyraCards — Oskar Östlind". `bash scripts/build.sh` går igenom utan fel. Docs, CASE-BANK och README uppdaterade.
...
- **2026-08-08** — **Kundnytt och Timringskurs lyfta till startsidan.** Båda casen är nu `featured`, vilket ger sex kort i startsidans rutnät: AvyraCards brett överst, NextWatch/Escape Room och JJ Bygg/Kundnytt i par, Timringskurs brett underst (orphan-regeln i `FeaturedWork.tsx` fångar det sista kortet automatiskt). Caset `bevakning` heter numera `kundnytt` rakt igenom — slug, titel och live-URL (`https://kundnytt.se` i stället för `rss-feed-lime.vercel.app`) — och bilden är omdöpt med `git mv` till `public/case/kundnytt.webp`. Ny skärmbild hämtad från kundnytt.se, som bytt till mörkt tema sedan förra bilden togs; uppskalad till exakt 1600 px så bildskriptets bredd-varning uteblir. **Observation värd att ta ställning till:** `timringskurs.nu` visar fortfarande Norrhed Skogs *gamla* sajt, inte den Oskar byggt — den nya ligger bara på `timringskurs.vercel.app`. Live-länken är därför oförändrad, och miniatyren är kvar från vercel-versionen. Peka om domänen först, byt länk sen. Verifierat: `scripts/case-images.mjs` plockar upp `kundnytt` utan slug-varning, `tsc --strict` går rent på `lib/projects.ts`. **Committat som `9b93091` men inte pushat** — varken molnburken eller den lokala Linux-VM:en når GitHub med skrivrättigheter. Kör `git push` själv. (`sharp` och en del git-operationer kraschar med bus error i den lokala VM:en mot OneDrive-mappen; låsfilerna som blir kvar ligger flyttade i `.git/_stale/`.)
